...
//	// 获取server的数量
//    nserver = array_n(&pool->server); // server_pool中有多个server
//    nlive_server = 0;
...
//			// auto_eject_hosts是一个boolean值，默认是false。
//			// 用于控制twemproxy是否应该根据server的连接状态重建群集。
...
//        uint32_t ncontinuum = nserver_continuum *  RANDOM_POINTS_PER_SERVER; // 每个server所占用的随机点数
